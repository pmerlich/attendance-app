...
- [x] **H-01** — Removed `'unsafe-inline'` from `script-src` in `worker/index.ts` (kept the `'unsafe-eval'` dev-only addition; `style-src` unchanged). Updated the two test assertions in `tests/rendered-html.test.mjs` accordingly, plus added `doesNotMatch` guards against it coming back. Verified: typecheck ✓, lint ✓, build ✓, 11/11 tests ✓ — including the live-rendered CSP header from the actual built Worker no longer containing `'unsafe-inline'` in `script-src`.
