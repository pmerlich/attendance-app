...
function NavigationChooser({ address, label = "ניווט" }: { address: string; label?: string }) {
  ...
  return <details className="navigation-choice">
    <summary aria-label={"בחירת אפליקציית ניווט אל " + address}><NavigationIcon /><span>{buttonLabel}</span><span className="navigation-chevron" aria-hidden="true">⌄</span></summary>
    <div className="navigation-menu" role="group" aria-label="בחירת אפליקציית ניווט">
      <a className="navigation-waze" href={navigationUrl("waze", address)} target="_blank" rel="noreferrer" aria-label={"פתיחה ב-Waze: " + address}><span className="navigation-provider-dot" />Waze</a>
      <a className="navigation-google" href={navigationUrl("google", address)} target="_blank" rel="noreferrer" aria-label={"פתיחה ב-Google Maps: " + address}><span className="navigation-provider-dot" />Google Maps</a>
    </div>
  </details>;
}
