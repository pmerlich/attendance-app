  ...
  return <>
    ...
    {selectedProject ? <section className="project-detail-card">
      ...
      <div className="project-detail-metrics">
        ...
        {canManage && <article><span>מצב כספי</span><strong>{financialStatus(selectedProject.expectedAmount, selectedProject.paidAmount)}</strong></article>}
      ...
      {listedProjects.length ? <div className="dashboard-project-list">{listedProjects.map((project) => {
        ...
        return <div key={project.id} className={"dashboard-project-card" + (isTimerProject ? " timer-running" : "")} role="button" tabIndex={0} aria-label={"פתיחת פרטי הפרויקט " + project.name} onClick={(event) => { if (!eventStartedFromControl(event.target)) selectProject(project); }} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); selectProject(project); } }}>
          ...
          <div className="project-card-meta">
            ...
            {isTimerProject && <span className="working-now"><i /> {accountMode === "solo" ? "טיימר פעיל" : "עובדים עכשיו"}</span>}
          </div>
...
const auditFieldLabels: Record<string, string> = { name: "שם", address: "כתובת", status: "מצב", amount: "סכום", projectId: "פרויקט", clientId: "לקוח", paidAt: "תאריך תשלום", incurredAt: "תאריך הוצאה", durationSeconds: "משך", billingType: "תמחור", fixedPrice: "מחיר גלובלי", hourlyRate: "תעריף שעתי", note: "הערה", fileName: "קובץ" };
...
function auditDetails(entry: AuditEntry) {
  try {
    ...
    return changed.slice(0, 4).map((key) => `${auditFieldLabels[key] ?? key}: ${String(after[key] ?? "—")}`).join(" · ");
  } catch {
